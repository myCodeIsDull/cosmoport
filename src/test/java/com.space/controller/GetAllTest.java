...
public class GetAllTest extends AbstractTest {
    ...
    public void getCountWithFiltersNameAfterMaxRating() throws Exception {
        ...
        ResultActions resultActions = this.mockMvc.perform(get("/rest/ships?name=nt&after=32188140000000&maxRating=3")
                .accept(MediaType.APPLICATION_JSON_UTF8))
                .andExpect(status().isOk());
        ...
        String contentAsString = result.getResponse().getContentAsString();
        List<ShipInfoTest>actual = mapper.readValue(contentAsString,typeReference);
        List<ShipInfoTest>expected = testsHelper.getShipInfosByMaxRating(3.,
                testsHelper.getShipInfosByAfter(32188140000000L,
                        testsHelper.getShipInfosByName("nt",
                                testsHelper.getAllShips())));
        assertEquals("Возвращается не правильный результат при запросе GET /rest/ships с параметрами minRating,minCrewSize и minSpeed.", expected, actual);
    }
}